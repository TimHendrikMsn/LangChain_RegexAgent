...
class Settings(BaseSettings):
    ...
    model_name: str = Field(
        default="gpt-4.1-mini", description="Name of the model to use for the agent"
    )
    temperature: float = Field(
        default=0.1, description="Sampling temperature for model output"
    )
    max_tokens: int = Field(
        default=2000, description="Maximum number of tokens to generate in responses"
    )
    timeout: int = Field(
        default=30, description="Request timeout for the model in seconds"
    )
    ...
    document_path: str = Field(
        default="data/document.txt", description="Path to the document to be processed"
    )
    chroma_path: str = Field(
        default="chroma", description="Path to the chroma database"
    )

    # --- Middleware Settings ---
    thread_limit: int = Field(
        default=20, description="Maximum tool calls across all runs in a thread"
    )
    run_limit: int = Field(
        default=6, description="Maximum tool calls per single invocation"
    )
