...
def test_invoke_all_matches():
    result = run_regex.invoke(
        {"pattern": r"\bgloom\b", "match_type": "all", "settings": test_settings}
    )
...
def test_invoke_first_match():
    result = run_regex.invoke(
        {"pattern": r"\bgloom\b", "match_type": "first", "settings": test_settings}
    )
    assert isinstance(result, dict)
    assert result["count"] == 2
...
def test_invoke_last_match():
    result = run_regex.invoke(
        {"pattern": r"\bgloom\b", "match_type": "last", "settings": test_settings}
    )
    assert isinstance(result, dict)
    assert result["count"] == 2
    assert result["matches"] == ["gloom"] * 1
...
def test_invoke_no_matches():
    result = run_regex.invoke(
        {"pattern": r"\bnonexistent\b", "match_type": "all", "settings": test_settings}
    )
    assert isinstance(result, dict)
...
def test_all_matches_count_and_values():
    result = run_regex.invoke(
        {"pattern": r"\bgl\w*", "match_type": "all", "settings": test_settings}
    )
...
def test_invoke_contextual_match():
    ...
    result = run_regex.invoke(
        {"pattern": pattern, "match_type": "all", "settings": test_settings}
    )
    assert isinstance(result, dict)
    assert result["count"] == 2
...
def test_invoke_case_insensitive_flag():
    ...
    result_no_flag = run_regex.invoke(
        {"pattern": r"\bgloom\b", "match_type": "all", "settings": test_settings}
    )
...
def test_invoke_multiline_flag():
    ...
    result_no_flag = run_regex.invoke(
        {"pattern": r"^Some", "match_type": "all", "settings": test_settings}
    )
    assert isinstance(result_no_flag, dict)
...
def test_invoke_dotall_flag():
    ...
    result_no_flag = run_regex.invoke(
        {"pattern": r"This.splitted", "match_type": "all", "settings": test_settings}
    )
    assert isinstance(result_no_flag, dict)
    assert result_no_flag["count"] == 0
...
def test_invoke_multiple_flags():
    ...
    result_no_flags = run_regex.invoke(
        {"pattern": r"This.*CONTAINS", "match_type": "all", "settings": test_settings}
    )
